...
module.exports = angga = async (angga, mek) => {
		try {
...
angga.on("CB:action,,battery", json => {
...
//━━━━━━━━━━━━━━━[ FUNCTION 2 ]━━━━━━━━━━━━━━━\\
			...
			const pushname = mek.key.fromMe ? angga.user.name : conts.notify || conts.vname || conts.name || 'No Name'                          
			...
			const group = angga.chats.array.filter(v => v.jid.endsWith('g.us'))
            const private = angga.chats.array.filter(v => v.jid.endsWith('s.whatsapp.net'))
            const totalchat = await angga.chats.all()
...
//━━━━━━━━━━━━━━━[ DATE ]━━━━━━━━━━━━━━━\\
        ...
        const isUrl = (url) => {
			...
			const reply = (teks) => {
			angga.sendMessage(from, teks, text, { thumbnail: gambar, sendEphemeral: true, quoted: mek, contextInfo: { forwardingScore: 508, isForwarded: true, externalAdReply:{title: `${jmn} WIB - ${week}`,body:"Developer anggaBot",previewType:"PHOTO",thumbnail:gambar,sourceUrl:`https://chat.whatsapp.com/GIA3iBpDCzOHPVICRgIqSE`}}})
		}
		const reply2 = (teks) => {
angga.sendMessage(from, teks, text, {quoted: mek, contextInfo: {"forwardingScore":999,"isForwarded":true},sendEphemeral: true})
}
			...
			if (budy.includes("https://chat.whatsapp.com/")) {
				...
				setTimeout(() => {
				angga.groupRemove(from, [kic]).catch((e) => { reply(`Jadikan Bot Sebagai Admin Untuk Menggunakan Perintah Ini!!`) })
				}, 0)
			   }
...
//━━━━━━━━━━━━━━━[ FAKE REPLY ]━━━━━━━━━━━━━━━\\
           ...
           const fakedoc = {key: {fromMe: false, participant: `${nomorowner}@s.whatsapp.net`, ...(from ? {remoteJid: "status@broadcast" } : {}) }, message: {documentMessage: {mimetype: 'application/octet-stream', title: `${ucapanFakereply}`, pageCount: 0, fileName: `Bot by ANGGA`, jpegThumbnail: fs.readFileSync(`./media/gambar/thumb.jpg`)}}}
...
const ftroly = {key : {fromMe:false,
    ...
    gam = img
    jadinya = await angga.prepareMessage(from, gam, MessageType.image)
    buttonMessagesI = {
    ...
    angga.sendMessage(from, buttonMessagesI, MessageType.buttonsMessage, {quoted: mek})
  }
    const sendMediaURL = async(to, url, text="", mids=[]) =>{
                ...
                download(url, filename, async function () {
                    ...
                    angga.sendMessage(to, media, type, { quoted: mek, mimetype: mime, caption: text,contextInfo: {"mentionedJid": mids}})
      ...
      kma = gam1;
      mhan = await angga.prepareMessage(from, kma, video);
      ...
      angga.sendMessage(id, buttonMessages, MessageType.buttonsMessage, {quoted: mek})
    };
        
        const sendButloc = async(id, text1, desc1, gam1, but = [], options = {}) => {
               let kma = gam1
               angga.sendMessage(id, {"contentText": text1,
               "footerText": desc1, 
               "buttons": but,
               "headerType": "LOCATION",
                       "locationMessage": {
      ...
      angga.sendMessage(id, buttonMessage, MessageType.buttonsMessage, {quoted: mek})
    };
...
return [...text.matchAll(/@([0-9]{5,16}|0)/g)].map(v => v[1] + '@s.whatsapp.net')
			...
			let authorname = angga.contacts[from] != undefined ? angga.contacts[from].vname || angga.contacts[from].notify : undefined	
			if (authorname != undefined) { } else { authorname = groupName }	
			
			function addMetadata(packname, author) {	
				if (!packname) packname = 'anggaBot'; if (!author) author = 'by angga';	
...
async function sendButLocation(id, text1, desc1, gam1, but = [], options = {}) {
            ...
            return angga.sendMessage(id, buttonMessages, MessageType.buttonsMessage, options)
        }       
        
        const sendFileFromUrl = async(link, type, options) => {
           hasil = await getBuffer(link)
	       angga.sendMessage(from, hasil, type, options).catch(e => {
	       fetch(link).then((hasil) => {
	       angga.sendMessage(from, hasil, type, options).catch(e => {
	       angga.sendMessage(from, { url : link }, type, options).catch(e => {
...
Congrats!! 🎉`
sendButMessage(from, `${lvlup}`, `Powered By angga`,
...
run = process.uptime() 
           angga.setStatus(`${namabot} Aktif Selama ${kyun(run)} | YouTube : anggaBOT`).catch((_)=>_);
...
const fakeText = (teks) => {
angga.sendMessage(from, teks, text, {quoted: froxx})
}
...
stst = await angga.getStatus(`${sender.split('@')[0]}@c.us`)
...
╾ _Browser : ${angga.browserDescription[1]}_
╾ _Server : ${angga.browserDescription[0]}_
╾ _Version : ${angga.browserDescription[2]}_
╾ _Handphone : ${angga.user.phone.device_manufacturer}_
...
//━━━━━━━━━━━━━━━[ ANIME FITUR ]━━━━━━━━━━━━━━━\\
            ...
            if (isBanned) return reply(mess.banned)
             ...
             cewe = await getBuffer(anu.result.female)
              ...
              angga.sendMessage(from, cowo, image, {quoted: froxx, caption: 'Cowo' })
              angga.sendMessage(from, cewe, image, {quoted: froxx, caption: 'Cewe' })
              break
             case 'waifu':
             case 'loli':
            case 'husbu':
            ...
            if (isBanned) return reply(mess.banned)
              ...
              imageMsg = ( await angga.prepareMessage(from, fs.readFileSync(`./${sender}.jpeg`), 'imageMessage', {thumbnail: Buffer.alloc(0)})).message.imageMessage
              buttonsMessage = {footerText:'©Created by angga', imageMessage: imageMsg,
              contentText:`NthnDevId It's Me`,buttons,headerType:4}
              prep = await angga.prepareMessageFromContent(from,{buttonsMessage},{quoted: froxx})
              anggarelayWAMessage(prep)
...
but = [
        ...
        sendButLocation(from, teks, `©Created by angga`, fakeimg, but, { thumbnail: Buffer.alloc(0) })
break
...
//━━━━━━━━━━━━━━━[ ANIMATION ANIMALS ]━━━━━━━━━━━━━━━\\
                   ...
                   angga.sendMessage(from, anu1, image, {caption: `Ini Rubah`, quoted: froxx})
                   break
                   case 'dog':  
                   if (isBanned) return reply(mess.banned)
                   anu = await fetchJson(`https://some-random-api.ml/img/dog`)
                   anu1 = await getBuffer(anu.link)
                   angga.sendMessage(from, anu1, image, {caption: `Ini Anjing`, quoted: froxx})
                   break
                   case 'cat':
                   if (isBanned) return reply(mess.banned)
                   anu = await fetchJson(`https://some-random-api.ml/img/cat`)
                   anu1 = await getBuffer(anu.link)
                   angga.sendMessage(from, anu1, image, {caption: `Ini Kucing`, quoted: froxx})
                   break
                   case 'panda':  
                   if (isBanned) return reply(mess.banned)
                   anu = await fetchJson(`https://some-random-api.ml/img/panda`)
                   anu1 = await getBuffer(anu.link)
                   angga.sendMessage(from, anu1, image, {caption: `Ini Panda`, quoted: froxx})
                   break
                   case 'panda2':  
                   if (isBanned) return reply(mess.banned)
                   ...
                   anu1 = await getBuffer(anu.link)
                   angga.sendMessage(from, anu1, image, {caption: `Ini Burung`, quoted: froxx})
                   break
                   case 'koala':  
                   if (isBanned) return reply(mess.banned)
                   anu = await fetchJson(`https://some-random-api.ml/img/koala`)
                   anu1 = await getBuffer(anu.link)
                   angga.sendMessage(from, anu1, image, {caption: `Ini Koala`, quoted: froxx})
                   break
...
//━━━━━━━━━━━━━━━[ RANDOM IMAGE ]━━━━━━━━━━━━━━━\\
                ...
                reply(mess.wait)
                    getBuffer(`https://api.lolhuman.xyz/api/random/${command}?apikey=${Lolhuman}`).then((gambar) => {
                        angga.sendMessage(from, gambar, image, { quoted: froxx })
                    })
                    break
                case 'chiisaihentai':
                case 'trap':
...
//━━━━━━━━━━━━━━━[ KODE BAHASA ]━━━━━━━━━━━━━━━\\
                     ...
                     case 'bahasa':
                     if (isBanned) return reply(mess.banned)
                    angga.sendMessage(from, bahasa(), text, { quoted:froxx })
                    break 
                    case 'kodenegara':
                    if (isBanned) return reply(mess.banned)
                    ...
                    case 'tts':
                    if (isBanned) return reply(mess.banned)
				    if (args.length < 1) return angga.sendMessage(from, 'Masukan Kode Bahasanya!!', text, {quoted: mek})
					const gtts = require('./lib/gtts')(args[0])
					if (args.length < 2) return angga.sendMessage(from, 'Masukan Text Nya?', text, {quoted: mek})
					dtt = body.slice(8)
					ranm = getRandom('.mp3')
					rano = getRandom('.ogg')
					dtt.length > 500
...
break
                case 'premiumlist':
				angga.updatePresence(from, Presence.recording) 
				teks = 'LIST USER PREMIUM :\n'
				for (let premm of prem) {
					teks += `~> @${premm.split('@')[0]}\n`
					}
					teks += `Total : ${prem.length}`
				angga.sendMessage(from, teks.trim(), extendedText, {quoted: froxx, contextInfo: {"mentionedJid": prem}})
				break
//━━━━━━━━━━━━━━━[ BANNED ]━━━━━━━━━━━━━━━\\
case 'ban':
if (!isOwner) return reply(mess.only.ownerB)
...
//━━━━━━━━━━━━━━━[ STICKER ]━━━━━━━━━━━━━━━\\
			...
			angga.sendMessage(from, wkwk, sticker, {quoted: froxx})
			break
			case 'amongus':
			if (isBanned) return reply(mess.banned)
			random = Math.floor(Math.random() * 6) + 1
			...
			break
					...
					if (isBanned) return reply(mess.banned)
						if (isMedia && !mek.message.videoMessage || isQuotedImage) {
							...
							const media = await angga.downloadAndSaveMediaMessage(encmedia, `./sticker/${sender}`)
							await ffmpeg(`${media}`)
									.input(media)
									...
									.on('end', function () {
										...
										exec(`webpmux -set exif ./sticker/data.exif ./sticker/${sender}.webp -o ./sticker/${sender}.webp`, async (error) => {
											...
											angga.sendMessage(from, fs.readFileSync(`./sticker/${sender}.webp`), sticker, {quoted: froxx})
											fs.unlinkSync(media)	
...
angga.sendMessage(from, mp4, video)
break
case 'mp3':
if (isBanned) return reply(mess.banned)
reply(mess.wait)
...
angga.sendMessage(from, mp3, audio)
break
case 'ytmp3':
if (isBanned) return reply(mess.banned)
reply(mess.wait)
...
angga.sendMessage(from, thum, image, { quoted: mek, caption: ini_txt })
res = await getBuffer(get.url_audio)
angga.sendMessage(from, res, audio)
break
case 'ytmp4':
if (isBanned) return reply(mess.banned)
reply(mess.wait)
...
angga.sendMessage(from, res, video)
break
case 'tiktok':
if (isBanned) return reply(mess.banned)
reply(mess.wait)
...
reply(mess.wait)
              ...
              angga.sendMessage(from, ini_video, video, { quoted: froxx })
              break
          case 'tiktokmp3': 
          if (isBanned) return reply(mess.banned)
           reply(mess.wait)
              ...
              angga.sendMessage(from, data, audio, { quoted: froxx })
              break
              case 'ytplaymp3':
              await axios.get(`https://api.zeks.xyz/api/ytplaymp3/2?apikey=${ZeksKey}&q=${q}`)
		     .then(res => {
			  angga.sendMessage(from, { url: res.data.result.link }, 'audioMessage', { mimetype: 'audio/mp4', quoted: froxx, contextInfo: { externalAdReply: { title: res.data.result.title, mediaType: 2, thumbnailUrl: res.data.result.thumb, mediaUrl: res.data.result.source }}})
})
              break
case 'lirik':
if (isBanned) return reply(mess.banned)
...
case 'bc': 
					...
					if (isMedia && !mek.message.videoMessage || isQuotedImage) {
						...
						for (let _ of anu) {
							angga.sendMessage(_.jid, bc, image, {quoted:mek,caption: `*「 PESAN SIARAN BOT 」*\n\n${body.slice(4)}`})}
             reply('Sukses Boadcast')
             } else {
             for (let _ of anu) {
             angga.sendMessage(_.jid, 
			{"contentText": `*「 BROADCAST 」*\n\n${body.slice(4)}`,
			"footerText": 'anggaBot by angga',
			"buttons": [
        ...
        if (isBanned) return reply(mess.banned)
             ...
             if (isMedia && !mek.message.videoMessage || isQuotedImage) {
             const encmedia = isQuotedImage ? JSON.parse(JSON.stringify(mek).replace('quotedM','m')).message.extendedTextMessage.contextInfo : mek
             bc = await angga.downloadMediaMessage(encmedia)
             for (let i of anu) {
             	tes = `${body.slice(4)}`
             	angga.sendMessage(i.jid, bc, { contentText: `${tes}`, footerText: `_Broadcast*_`, buttons: [{buttonId: `${prefix}owner`,buttonText:{displayText: 'OWNER'},type:1}],headerType: 'LOCATION', locationMessage: { degreesLatitude: '', degreesLongitude: '', jpegThumbnail: bctlah, contextInfo: {mentionedJid: [sender]}}}, 'buttonsMessage')}
             reply('Suksess Broadcast')
             } else {
             for (let i of anu) {
             	textt = `${body.slice(4)}`
             angga.sendMessage(i.jid, { contentText: `${textt}`, footerText: `_Broadcast*_`, buttons: [{buttonId: `${prefix}owner`,buttonText:{displayText: 'OWNER'},type:1}],headerType: 'LOCATION', locationMessage: { degreesLatitude: '', degreesLongitude: '', jpegThumbnail: bctlah, contextInfo: {mentionedJid: [sender]}}}, 'buttonsMessage')}
             reply('Sukses broadcast')}
             break
...
angga.sendMessage(from, buff, image, {quoted: froxx, caption : teks})
break
...
group = await angga.groupMetadata(from);
...
await angga.sendMessage(from, options, text)
break
case 'wa.me':
case 'wame':
if (isBanned) return reply(mess.banned)
...
const vcard = 'BEGIN:VCARD\n'  
            ...
            + 'END:VCARD'  
  angga.sendMessage(from, {displayname: "angga", vcard: vcard}, MessageType.contact, { quoted: mek})
...
{buttonId: `${prefix}script`, buttonText: { displayText: `SCRIPT`, }, type: 1, }
                 ...
                 if (isBanned) return reply(mess.banned)
					...
					angga.sendMessage(`${nomorowner}@s.whatsapp.net`, options, text, {quoted: mek})
...
if (isBanned) return reply(mess.banned)
					...
					if (args.length > 300) return angga.sendMessage(from, 'Maaf Teks Terlalu Panjang, Maksimal 300 Teks', msgType.text, {quoted: mek})
					stod = `${sender}`
					const repo = `*[REPORT]*\nNomor : @${stod.split('@')[0]}\nPesan : ${inipesannya}`
							var options = {
							text: repo,
      ...
      if (isBanned) return reply(mess.banned)
             ...
             let totalgerup = angga.chats.array.filter(u => u.jid.endsWith('@g.us')).map(u => u.jid)
        ...
        if (isBanned) return reply(mess.banned)
            ...
            var response = await angga.acceptInvite(codeInvite)
        ...
        if (isBanned) return reply(mess.banned)
             ...
             fak = angga.query({ json: ['action', 'invite', link],
             expect200: true })
...
//━━━━━━━━━━━━━━━[ GROUP ]━━━━━━━━━━━━━━━\\
 ...
 if (!isGroup) return reply(mess.only.group)
             ...
             let online = [...Object.keys(angga.chats.get(ido).presences), angga.user.jid]
             angga.sendMessage(from, 'List Online:\n' + online.map(v => '- @' + v.replace(/@.+/, '')).join `\n`, text, { quoted: mek, contextInfo: { mentionedJid: online }})
             } catch (e) {
             reply(`${e}`)
}
             break
  ...
  if (!isGroup) return reply(mess.only.group)
              ...
              angga.groupUpdateSubject(from, q)
  ...
  if (!isGroup) return reply(mess.only.group)
              ...
              angga.groupUpdateDescription(from, q)
             .then((res) => reply(jsonformat(res)))
             .catch((err) => reply(jsonformat(err)))
              break
       case 'setppgrup':
       ...
       if (isBanned) return reply(mess.banned)
           if (!isGroup) return reply(mess.only.group)
              ...
              let media = await angga.downloadMediaMessage(encmedia)
              angga.updateProfilePicture(from, media)
             .then((res) => reply(jsonformat(res)))
             .catch((err) => reply(jsonformat(err)))
              } else {
		...
		if (!isBotGroupAdmins) return reply(mess.only.Badmin)
		 ...
		 for (let mem of groupMembers) {
		 	...
		 	members_id.push(mem.jid)
		 		  ...
		 		  angga.groupDemoteAdmin(from, members_id)
	...
	if (isBanned) return reply(mess.banned)
		 		    	...
		 		    	for (let mem of groupMembers) {
		 		    	 	...
		 		    	 	members_id.push(mem.jid)
		 		    	 	 	   ...
		 		    	 	 	   angga.groupMakeAdmin(from, members_id)
             
		 		    	 	 	      break
case 'group':
if (isBanned) return reply(mess.banned)
if (!isGroup) return reply(mess.only.group)
    nthn = '©Created by angga'
...
if (isBanned) return reply(mess.banned)
  ...
  if (!mek.key.fromMe && !isGroupAdmins) return reply("Hanya Bisa Dilakukan Oleh Bot Dan Admin Group Saja");
   ...
   if (isBanned) return reply(mess.banned)                
					...
					var lahh = await angga.groupMetadata(from)
					...
					angga.sendMessage(from, value, text, {quoted: ftag, contextInfo: { mentionedJid: mem }})
					break;
									case 'tagall':
									if (isBanned) return reply(mess.banned)
if (!isGroup) return reply(mess.only.group)
        ...
        break;
                ...
                if (isBanned) return reply(mess.banned)
					...
					if (Number(args[0]) === 1) {
						...
						angga.sendMessage(from,`*ANTILINK* Sudah Aktif. Semua Member Dilarang Mengirim Link Group, Karena Bot Akan Mengeluarkan Target Jika Target Tersebut Mengirimkan Link Group`, text)
					} else if (Number(args[0]) === 0) {
...
angga.sendMessage(from, 'Oke Sudah Aktif', text, {quoted: mek})
                  }
}
if (budy.startsWith('x')){
try {
return angga.sendMessage(from, JSON.stringify(eval(budy.slice(2)),null,'\t'),text, {quoted: mek})
} catch(err) {
e = String(err)
reply(e)
}
